...
import { Activity, BedDouble, Stethoscope, LayoutGrid, LogOut, Bell, Users, Calendar, AlertTriangle, ClipboardList, Heart } from "lucide-react";
...
function StatCard({ label, value, total, color, pct }) {
  return (
    <div style={s.statCard}>
      ...
      <div style={{ fontSize:"11px", color:"#475569", marginTop:"3px" }}>de {total} disponibles</div>
...
function Sidebar({ rol, initials, nombre }) {
  const navAdmin = [
    ...
    { icon:BedDouble, label:"Espacios" },
    { icon:Calendar, label:"Cronogramas" },
    { icon:Users, label:"Usuarios" },
    { icon:ClipboardList, label:"Reportes" },
  ];
  const navMedico = [
    { icon:LayoutGrid, label:"Dashboard", active:true },
  ...
  return (
    <div style={s.sidebar}>
      ...
      <div style={s.nav}>
        ...
        {items.map(i => <NavItem key={i.label} icon={i.icon} label={i.label} active={i.active} />)}
      </div>
  ...
  return (
    <>
      <div style={s.statsGrid}>
        <StatCard label="Camas" value="42" total="80" color="#1d9e75" pct={52} />
        <StatCard label="Salas de cirugía" value="3" total="6" color="#378ADD" pct={50} />
        <StatCard label="Consultorios" value="18" total="24" color="#EF9F27" pct={75} />
        <StatCard label="UCI" value="2" total="10" color="#E24B4A" pct={80} />
      </div>
      <div style={s.twoCol}>
        <div style={s.card}>
          <div style={s.cardHeader}>
            ...
            <span style={{ fontSize:"11px", color:"#5dcaa5", cursor:"pointer" }}>Ver todos</span>
          </div>
          {ESPACIOS.map(e => (
  ...
  return (
    <>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,minmax(0,1fr))", gap:"10px" }}>
        <StatCard label="Camas disponibles" value="42" total="80" color="#1d9e75" pct={52} />
        <StatCard label="Salas disponibles" value="3" total="6" color="#378ADD" pct={50} />
        <StatCard label="Consultorios libres" value="18" total="24" color="#EF9F27" pct={75} />
      </div>
      <div style={s.card}>
        <div style={s.cardHeader}>
...
function EnfermeroDashboard() {
  return (
    <>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(3,minmax(0,1fr))", gap:"10px" }}>
        <StatCard label="Mis camas asignadas" value="8" total="12" color="#378ADD" pct={66} />
        <StatCard label="Pacientes activos" value="6" total="8" color="#1d9e75" pct={75} />
        <StatCard label="Pendientes de egreso" value="2" total="8" color="#EF9F27" pct={25} />
      </div>
      <div style={s.twoCol}>
        <div style={s.card}>
          <div style={s.cardHeader}>
        ...
        <div style={s.card}>
          <div style={s.cardHeader}>
            ...
            <span style={{ fontSize:"11px", color:"#5dcaa5", cursor:"pointer" }}>Actualizar</span>
          </div>
          {ESPACIOS.filter(e => e.tipo === "Cama").map(e => (
            <div key={e.id} style={s.row}>
              <div style={{ width:"7px", height:"7px", borderRadius:"50%", background: pillColor(e.estado).color, flexShrink:0 }} />
              <div style={{ flex:1 }}>
                <div style={{ fontSize:"12px", color:"#e2e8f0" }}>{e.nombre}</div>
    ...
    <div style={s.dash}>
      ...
      <div style={s.main}>
        ...
        <div style={s.content}>
          ...
          {rol === "Médico" && <MedicoDashboard />}
          {rol === "Enfermero" && <EnfermeroDashboard />}
        </div>
      </div>
    </div>
  );
